function varargout = GIAcompare22(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @GIAcompare22_OpeningFcn, ...
                   'gui_OutputFcn',  @GIAcompare22_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GIAcompare22 is made visible.
function GIAcompare22_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GIAcompare22 (see VARARGIN)
...
% UIWAIT makes GIAcompare22 wait for user response (see UIRESUME)
% uiwait;


% --- Outputs from this function are returned to the command line.
function varargout = GIAcompare22_OutputFcn(hObject, eventdata, handles) 
...
[theirpd ourpd] = Compute2Distibution(handles.data,handles.option);
...
tmp1 = uigetdir('L:\Lab Member data files\Elden\Qiang bulb imaging data');
if tmp1==0
    return;
end
ProjectsDir = {};
...
function [theirpd ourpd] = Compute2Distibution(data,option)
%% helper function to compute the 2 pdist distribution
%% 
allOurCAS = {};
for z=1:data.cproj
    for zz=1:length(data.OurCAS{z})
        allOurCAS{end+1} = data.OurCAS{z}{zz};
    end    
end
allOurCASUnique = unique(allOurCAS);
[~,LOC] = ismember(allOurCASUnique,data.CAS);
ourOdorDB = data.odorDB(LOC,:);
ourOdorDB = zscore(ourOdorDB);
...
for z=1:data.cproj
    ...
    %OdorAbr = tOurAbr(indHasDesc & indResponse);
    %
    [~,LOC2] = ismember(data.OurCAS{z},allOurCASUnique);
    tmp = ourOdorDB(LOC2,:);
%    tmp0 = ComputePd(tmp00,option.Des);            
%    tmp0 = zscore(tmp00);            
%   tmp = tmp00(OurCasLoc(indHasDesc & indResponse),:);
...
% %     % use the following to produce a customized subset of descriptors
% %     % the following yields r=0.85
% %    a=[303 1140 1061 428 363 926 344 683 1013 1550 332 987 1499 1152 521];    
%     % r=0.6871
% %    a=[900 1558 197 432 1526 207 1558 468 947 1193 942 468 1558 513 1183 901 714 575 468 763 1558];    
%     % r = 0.6641
%     a = [1558 553 1069 498 432 1558 997 1231 1254 1069 1580 819 1558 384 1646 334 935 1069 30]

%    TheirRep = tmp(:,a);
    
    OurPd = ComputePd(OurRep,option.Res);
    OurPd = OurPd(~isnan(OurPd));
    if sum(isnan(OurPd))>0
        disp('zero resp?')
    end
    TheirPd = ComputePd(TheirRep,option.Des);
    TheirPd = TheirPd(~isnan(OurPd));
    if length(OurPd)~=length(TheirPd)
        disp('here wrong');
    end    
    ourpd = [ourpd OurPd];
